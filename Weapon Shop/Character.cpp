...
void Character::UseWeapon(Character& target)
{
    if (GetLifePoints() > 0)
    {
        ...
        else
        {
            ...
            int modifier = 5;
            int nrDice = 1, nrSides = 6, result = 0;
            ...
            cout << _charaFirstName << " used their bare fists to deal " << damage << " damage to " << target.GetFirstName() << " and has received " << damage / 2 << " damage" << endl;
